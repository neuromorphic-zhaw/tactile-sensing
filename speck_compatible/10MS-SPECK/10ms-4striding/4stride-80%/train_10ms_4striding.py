...
class Speck9LayerTrainer(nn.Module):
    ...
    def forward(self, x):
        ...
        # This condenses 1,331 steps into ~333 steps, speeding up training by ~400%!
        x = x[:, ::4, :, :, :] 
        new_timesteps = x.shape[1]
        
        # Now unroll the much smaller timeline safely into memory
...
for epoch in range(EPOCHS):
    ...
    for i, (data, target) in enumerate(train_loader):
        ...
        # 2. 🎯 CRITICAL: Unscale gradients BEFORE clipping
        scaler.unscale_(optimizer)
        
        # 3. 🛡️ THE SHIELD: Clip the gradients to a maximum norm of 1.0
        torch.nn.utils.clip_grad_norm_(model.parameters(), max_norm=1.0)
        
        # 4. Step the optimizer and update the dynamic scale factor
    ...
    # ====================================================================
    #  AUTOMATED NAN CIRCUIT BREAKER LATCH
    # ====================================================================
    if np.isnan(avg_train_loss) or np.isinf(avg_train_loss):
        print(f"🚨 [CATASTROPHIC FAILURE] Epoch {epoch+1:02d} produced NaN/Inf loss! Aborting run to protect cluster space.", flush=True)
        break
    # ====================================================================
