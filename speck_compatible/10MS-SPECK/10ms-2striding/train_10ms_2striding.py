...
BATCH_SIZE = 7         
NUM_WORKERS = 4  # Balanced worker pool to prevent Host RAM OOM-kills on cluster partition       
EPOCHS = 100
LEARNING_RATE = 2e-4    
WEIGHT_DECAY = 1e-4  # 🎯 NEW: Smooths weight shifts & prevents dead-neuron latching (0.0000 loss)
...
class Speck9LayerTrainer(nn.Module):
    ...
    def forward(self, x):
        ...
        # ⚡ STRIDE THE TIME DIMENSION (Take every 2nd frame)
        # 🎯 OPTIMIZATION: Double the temporal density to preserve fine mechanical signals
        x = x[:, ::2, :, :, :] 
        new_timesteps = x.shape[1]
        
        # Unroll the higher timeline timeline safely into memory
        ...
        # Return the mean firing rate over the condensed window
        return out_spikes.mean(dim=1)


model = Speck9LayerTrainer(target_batch_size=BATCH_SIZE).to(DEVICE)
# 🎯 OPTIMIZATION: Switched to AdamW to handle regularization with Weight Decay safely
...
for epoch in range(EPOCHS):
    ...
    for i, (data, target) in enumerate(train_loader):
        ...
        # 2. Unscale gradients before truncation
        scaler.unscale_(optimizer)
        
        # 3. Clip gradients to maximum norm of 1.0 to protect against NaNs
        torch.nn.utils.clip_grad_norm_(model.parameters(), max_norm=1.0)
        
        # 4. Step optimizer and update dynamic scale factor
        scaler.step(optimizer)
        scaler.update()
        
        batch_loss_val = loss.item()
        
        # 🛡️ LIVE CIRCUIT BREAKER: Check immediately at the batch level
    ...
    # Escape outer framework loop if inner loop encountered a NaN break
